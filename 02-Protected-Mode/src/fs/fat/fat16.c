...
static void fat16_init_private(struct disk *disk, struct fat_private *fat_private)
{
    ...
    fat_private->cluster_read_stream = disk_streamer_new(disk->id);
    fat_private->fat_read_stream = disk_streamer_new(disk->id);
    fat_private->directory_stream = disk_streamer_new(disk->id);
}
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
    ...
    disk_streamer_seek(stream, directory_start_pos);
...
{
    while (*in != 0x00 && *in != 0x20) // 0x20: space
    {
        **out = *in;
        *out += 1;
        in += 1;
...
// filename: test, ext: txt -> test.txt
...
// starting cluster와 offset이용해 올바른 cluster 계산
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        // secotr이 bad sector로 마킹 되었는지 확인
...
{
    // data cluster는 root directory이후 부터 시작, cluster는 cluster 2부터 시작
...
struct fat_item *fat16_new_fat_item_for_directory_item(struct disk *disk, struct fat_directory_item *item)
{
    ...
    if (item->attribute & FAT_FILE_SUBDIRECTORY) // 하위 경로인지 확인
    {
...
struct fat_item *fat16_get_directory_entry(struct disk *disk, struct path_part *path)
{
    ...
    struct fat_item *root_item = fat16_find_item_in_directory(disk, &fat_private->root_directory, path->part); // ex. 0:/test.txt -> test.txt , 0:/aa/test.txt -> aa/
